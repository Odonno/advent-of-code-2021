fn main1() {
    let input = "gabfed acbdfg cd gebcd gdfecb bgfdcae ecdf dcb gdbfe gaceb | dcb dgfceab cbfdeg edcf
...
decfb bc gfedc fcagbd bgec ecgdabf degfcb ebafd afcdeg bcd | aefbd fdceg dcb efgcbd";
    ...
    for line in lines {
        ...
        match second_part {
            Some(p) => {
                let digit_codes = p.split_whitespace();

                for digit_code in digit_codes {
                    let length = digit_code.len();
        
                    if length == 2 || length == 3 || length == 4 || length == 7 {
                        instances_of_digits += 1;
                    }
                }
            },
