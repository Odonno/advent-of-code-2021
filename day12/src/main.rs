...
tf-EU";
    ...
    for relation in from_start_relations {
        let path = Path {
            ...
            has_already_visited_single_small_cave: false,
        };
    ...
    loop {
        ...
        for working_path in current_working_paths {
            ...
            let new_cave_relations = &cave_relations
                ...
                .map(|cave_relation| {
                    ...
                    if is_from {
                        ...
                        if cave_relation.to.name == "start" {
                            return None;
                        }

                        let is_already_in_path = working_path.is_already_in_path(&cave_relation.to);

                        if !is_already_in_path {
                            return Some(cave_relation.to.clone());
                        }

                        if !working_path.has_already_visited_single_small_cave {
                            return Some(cave_relation.to.clone());
                        }
                    }
                    if is_to {
                        ...
                        if cave_relation.from.name == "start" {
                            return None;
                        }

                        let is_already_in_path =
                            working_path.is_already_in_path(&cave_relation.from);
            ...
            for cave in new_cave_relations {
                ...
                if !new_path.has_already_visited_single_small_cave {
                    let has_now_visited_single_small_cave = new_path
                        .caves
                        .iter()
                        .filter(|c| c.cave_type == CaveType::Small)
                        .filter(|c| c.name == cave.name)
                        .count()
                        == 2;

                    if has_now_visited_single_small_cave {
                        new_path.has_already_visited_single_small_cave = true;
                    }
                }
...
struct Path {
    caves: Vec<Cave>,
    has_already_visited_single_small_cave: bool,
}

impl Path {
    fn is_already_in_path(&self, cave: &Cave) -> bool {
        return self.caves.iter().any(|c| c.name == cave.name);
    }
}
